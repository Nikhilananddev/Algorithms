...
public class DeleteTreeNodes {
    ...
    public static int deleteTreeNodes(int nodes, int[] parent, int[] value) {
        ...
        while (!workQ.isEmpty()) {
            ...
            if (p != -1) {
                --directChildren[p];
                if (directChildren[p] == 0)
                    workQ.add(p); // enqueue parent since all its direction children were enqueued
                value[p] += value[n];
                // contribute node count only if n will not be deleted
                subtreeNodes[p] += value[n] != 0 ? subtreeNodes[n] : 0;
            }
            if (value[n] == 0) {
                ans -= subtreeNodes[n]; // delete this subtree
            }
        }
        return ans;
    }
